    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                flash('Logowanie pomyślne!', category='success')
                login_user(user, remember=True)
                return redirect(url_for('views.home'))
            else:
                flash('Nieprawidłowe hasło.Spróbuj ponownie.', category='error')
        else:
...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Imię musi mieć więcej niż 1 znak', category='error')
